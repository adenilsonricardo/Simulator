...
public class MainActivity extends AppCompatActivity {
    ...
    private void setupHttpClient() {
        Retrofit retrofit = new Retrofit.Builder()
                .baseUrl("https://adenilsonricardo.github.io/matches-simulator-api/")
            ...
            view.animate().rotation(360).setDuration(500).setListener(new AnimatorListenerAdapter() {
                ...
                public void onAnimationEnd(Animator animation) {
                    ...
                    for (int i = 0; i < matchesAdapter.getItemCount(); i++){
                        ...
                        match.getHomeTeam().setScore(random.nextInt(
                                match.getHomeTeam().getStars() + 1));
                        match.getAwayTeam().setScore(random.nextInt(
                                match.getAwayTeam().getStars() + 1));
                        matchesAdapter.notifyItemChanged(i);
                    }
                }
            });
            //throw new RuntimeException("Testes Crashlytics");
        });
    }
